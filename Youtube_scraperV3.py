        ...
        try:
            ...
            for index, item in enumerate(video_items):
                try:
                    ...
                    # --- New logic: Extract description and channel icon ---
                    try:
                        video_page = browser.new_page()
                        video_page.goto(video_info["url"])
                        video_page.wait_for_load_state('domcontentloaded')
                        time.sleep(2)
                        # Try to click the 'more' button to expand description
                        try:
                            expand_btns = video_page.locator('ytd-text-inline-expander#description-inline-expander tp-yt-paper-button#expand')
                            ...
                            log.debug(f"No expand button or failed to click: {e}")
                        # Extract description
                        try:
                            desc_elem = video_page.locator('ytd-text-inline-expander#description-inline-expander')
                            raw_desc = desc_elem.text_content().strip()
                            # Clean the description using LLM
                            try:
                                system_prompt = (
                                    "You are a helpful assistant. "
                                    "Rewrite the following YouTube video description as a single, natural, detailed, and concise summary. "
                                    "Do not start with phrases like 'Here's the cleaned description' or 'Summary:'. "
                                    "Remove all timestamps, repeated words, and unrelated text. "
                                    "Make it flow as a perfect, human-written description about the video content."
                                )
                                user_prompt = raw_desc
                                cleaned_desc, _ = llm_response(system_prompt, user_prompt)
                                video_info["description"] = cleaned_desc.strip()
                            except Exception as e:
                                log.warning(f"Failed to clean description with LLM: {e}")
                                video_info["description"] = raw_desc
                        except Exception as e:
                            log.warning(f"Failed to extract description: {e}")
                            video_info["description"] = None
                        # Extract channel icon
                        try:
                            icon_elem = video_page.locator('ytd-video-owner-renderer #avatar img').first
                            video_info["channel_icon"] = icon_elem.get_attribute('src')
                        except Exception as e:
                            log.warning(f"Failed to extract channel icon: {e}")
                            video_info["channel_icon"] = None
                        video_page.close()
                    except Exception as e:
                        log.warning(f"Failed to extract description/channel icon for video: {e}")
                        video_info["description"] = None
                        video_info["channel_icon"] = None
                    # --- End new logic ---
